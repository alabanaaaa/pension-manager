                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-neutral-50">
                      ...
                      <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Employees</th>
                      <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Employee Total</th>
                      <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Employer Total</th>
                      <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Grand Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-50">
                    {breakdown.map((b, i) => (
                      <tr key={i} className="hover:bg-neutral-50/50 transition-colors">
                        <td className="px-6 py-4 font-medium text-neutral-900">{b.period}</td>
                        <td className="px-6 py-4 text-right text-neutral-500">{b.employee_count}</td>
                        <td className="px-6 py-4 text-right font-mono text-xs text-neutral-600">KES {(b.employee_total / 100).toLocaleString()}</td>
                        <td className="px-6 py-4 text-right font-mono text-xs text-neutral-600">KES {(b.employer_total / 100).toLocaleString()}</td>
                        <td className="px-6 py-4 text-right font-mono text-xs font-semibold text-neutral-900">KES {(b.grand_total / 100).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-neutral-50">
                    ...
                    <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Employee YTD</th>
                    <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Employer YTD</th>
                    <th className="text-right px-6 py-3.5 font-medium text-neutral-400 text-xs uppercase tracking-wider">Total YTD</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-50">
                  {ytd.slice(0, 20).map((y, i) => (
                    <tr key={i} className="hover:bg-neutral-50/50 transition-colors">
                      <td className="px-6 py-4 font-medium text-neutral-900">{y.full_name}</td>
                      <td className="px-6 py-4 text-right font-mono text-xs text-neutral-600">KES {(y.employee_ytd / 100).toLocaleString()}</td>
                      <td className="px-6 py-4 text-right font-mono text-xs text-neutral-600">KES {(y.employer_ytd / 100).toLocaleString()}</td>
                      <td className="px-6 py-4 text-right font-mono text-xs font-semibold text-neutral-900">KES {(y.total_ytd / 100).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
